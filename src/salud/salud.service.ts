...
export class SaludService {
  ...
  async createEvento(empresaId: bigint, dto: CreateEventoSanitarioDto) {
    ...
    const evento = await this.prisma.eventos_sanitarios.create({
      data: data as Parameters<
        typeof this.prisma.eventos_sanitarios.create
      >[0]['data'],
      include: {
  ...
  async updateEvento(
    ...
    if (dto.id_enfermedad !== undefined) {
      data.id_enfermedad = dto.id_enfermedad
        ? parseBigInt(dto.id_enfermedad, 'id_enfermedad')
        : null;
    }
  ...
  async createTratamiento(empresaId: bigint, dto: CreateTratamientoDto) {
    ...
    if (dto.id_evento_sanitario) {
      tratamientoData.id_evento_sanitario = parseBigInt(
        dto.id_evento_sanitario,
        'id_evento_sanitario',
      );
    }

    const tratamiento = await this.prisma.tratamientos.create({
      data: tratamientoData as Parameters<
        typeof this.prisma.tratamientos.create
      >[0]['data'],
    });
  ...
  async getAlertas(empresaId: bigint) {
    ...
    const alertas = retirosActivos.map((r) => ({
      ...
      dias_restantes: Math.ceil(
        (r.fecha_fin.getTime() - hoy.getTime()) / (1000 * 60 * 60 * 24),
      ),
    }));
  ...
  async getRestricciones(empresaId: bigint, animalId: bigint) {
    ...
    const retiroLeche = retirosActivos.find(
      (r) =>
        r.tipos_retiro.codigo === 'leche' || r.tipos_retiro.codigo === 'LECHE',
    );
    const retiroCarne = retirosActivos.find(
      (r) =>
        r.tipos_retiro.codigo === 'carne' || r.tipos_retiro.codigo === 'CARNE',
    );
    ...
    const getRetiroInfo = (retiro: typeof retiroLeche) => {
      ...
      return {
        ...
        dias_restantes: Math.ceil(
          (retiro.fecha_fin.getTime() - hoy.getTime()) / (1000 * 60 * 60 * 24),
        ),
      };
    };
    ...
    return {
      ...
      hasta:
        retirosActivos.length > 0
          ? retirosActivos.reduce(
              (max, r) => (r.fecha_fin > max ? r.fecha_fin : max),
              retirosActivos[0].fecha_fin,
            )
          : null,
      notas:
        retirosActivos
          .map(
            (r) =>
              `${r.tipos_retiro.nombre}: ${r.tratamientos.medicamentos.nombre} hasta ${r.fecha_fin.toISOString().split('T')[0]}`,
          )
          .join('; ') || null,
    };
  }
