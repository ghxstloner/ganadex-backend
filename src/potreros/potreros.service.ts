...
export class PotrerosService {
  ...
  async findAll(empresaId: bigint, query: QueryPotreroDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.potreros.findMany({
        ...
        // [Punto 3] Ordenar por created_at DESC (lo más nuevo primero)
        orderBy: {
          created_at: 'desc',
        },
        include: {
  ...
  async create(empresaId: bigint, dto: CreatePotreroDto) {
    ...
    if (
      ...
      dto.geometry.length > 0
    ) {
      data.geometry = dto.geometry;
    }
    if (dto.id_tipo_potrero) {
      data.id_tipo_potrero = parseBigInt(
        dto.id_tipo_potrero,
        'id_tipo_potrero',
      );
    }
    ...
    if (dto.estado) {
      const estadoPotrero = await this.prisma.estados_potreros.findFirst({
        where: {
          OR: [{ codigo: dto.estado }, { nombre: dto.estado }],
          activo: true,
        },
      });
      if (estadoPotrero) {
  ...
  async update(empresaId: bigint, id: bigint, dto: UpdatePotreroDto) {
    ...
    if (dto.area_hectareas !== undefined) {
      data.area_hectareas = dto.area_hectareas
        ? parseFloat(dto.area_hectareas)
        : null;
    }
    ...
    if (dto.geometry !== undefined) {
      data.geometry =
        dto.geometry && Array.isArray(dto.geometry) && dto.geometry.length > 0
          ? dto.geometry
          : null;
    }
    if (dto.id_tipo_potrero !== undefined) {
      data.id_tipo_potrero = dto.id_tipo_potrero
        ? parseBigInt(dto.id_tipo_potrero, 'id_tipo_potrero')
        : null;
    }
    // El frontend envía estado como string, buscar por codigo o nombre y resolver a id_estado_potrero
    // estados_potreros es un catálogo global (sin empresa_id)
    if (dto.estado !== undefined) {
      if (dto.estado) {
        ...
        data.id_estado_potrero = estadoPotrero
          ? estadoPotrero.id_estado_potrero
          : null;
      } else {
    ...
    if (dto.capacidad_animales !== undefined) {
      data.capacidad_animales = dto.capacidad_animales
        ? parseFloat(dto.capacidad_animales)
        : null;
    }

    const potrero = await this.prisma.potreros.update({
      where: {
        id_potrero_empresa_id: { id_potrero: id, empresa_id: empresaId },
      },
      data,
      include: {
        fincas: { select: { nombre: true } },
  ...
  private mapPotrero(p: Record<string, unknown>) {
    ...
    return {
      ...
      estado:
        potrero.estados_potreros?.codigo ??
