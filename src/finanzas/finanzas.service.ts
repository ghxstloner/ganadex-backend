...
export class FinanzasService {
    ...
    async findAllTransacciones(empresaId: bigint, query: QueryTransaccionDto) {
        ...
        if (query.id_categoria) {
            where.id_categoria_financiera = parseBigInt(query.id_categoria, 'id_categoria');
        }
    ...
    async createTransaccion(empresaId: bigint, dto: CreateTransaccionDto) {
        const transaccion = await this.prisma.transacciones_financieras.create({
            data: {
                ...
                id_tipo_transaccion: parseBigInt(dto.id_tipo_transaccion, 'id_tipo_transaccion'),
                id_categoria_financiera: parseBigInt(dto.id_categoria_financiera, 'id_categoria_financiera'),
                monto: parseFloat(dto.monto),
                descripcion: dto.descripcion ?? null,
                id_tercero: dto.id_tercero ? parseBigInt(dto.id_tercero, 'id_tercero') : null,
            },
        });
    ...
    async updateTransaccion(empresaId: bigint, id: bigint, dto: UpdateTransaccionDto) {
        ...
        if (dto.id_tipo_transaccion !== undefined) {
            data.id_tipo_transaccion = parseBigInt(dto.id_tipo_transaccion, 'id_tipo_transaccion');
        }
        if (dto.id_categoria_financiera !== undefined) {
            data.id_categoria_financiera = parseBigInt(dto.id_categoria_financiera, 'id_categoria_financiera');
        }
    ...
    async getCategoriasFinancieras(empresaId: bigint) {
        const categorias = await this.prisma.categorias_financieras.findMany({
            ...
            orderBy: { orden: 'asc' },
            include: { tipos_transaccion: { select: { codigo: true, nombre: true } } },
        });
