...
export class MovimientosService {
    ...
    async findAll(empresaId: bigint, query: QueryMovimientoDto) {
        ...
        if (query.fecha_desde || query.fecha_hasta) {
            ...
            if (query.fecha_desde) {
                (where.fecha_hora as Record<string, unknown>).gte = new Date(query.fecha_desde);
            }
            if (query.fecha_hasta) {
                (where.fecha_hora as Record<string, unknown>).lte = new Date(query.fecha_hasta);
            }
        }
        ...
        const [data, total] = await Promise.all([
            this.prisma.movimientos_animales.findMany({
                ...
                include: {
                    ...
                    lotes_movimientos_animales_lote_origen_id_empresa_idTolotes: { select: { nombre: true } },
                    lotes_movimientos_animales_lote_destino_id_empresa_idTolotes: { select: { nombre: true } },
                    potreros_movimientos_animales_potrero_origen_id_empresa_idTopotreros: { select: { nombre: true } },
                    potreros_movimientos_animales_potrero_destino_id_empresa_idTopotreros: { select: { nombre: true } },
                },
            }),
    ...
    async create(empresaId: bigint, dto: CreateMovimientoDto) {
        ...
        if (dto.lote_destino_id) {
            data.lote_destino_id = parseBigInt(dto.lote_destino_id, 'lote_destino_id');
        }
        if (dto.potrero_origen_id) {
            data.potrero_origen_id = parseBigInt(dto.potrero_origen_id, 'potrero_origen_id');
        }
        if (dto.potrero_destino_id) {
            data.potrero_destino_id = parseBigInt(dto.potrero_destino_id, 'potrero_destino_id');
        }
        if (dto.id_motivo_movimiento) {
            data.id_motivo_movimiento = parseBigInt(dto.id_motivo_movimiento, 'id_motivo_movimiento');
        }

        const movimiento = await this.prisma.movimientos_animales.create({
            data: data as Parameters<typeof this.prisma.movimientos_animales.create>[0]['data'],
            include: {
                animales: { select: { nombre: true, id_animal: true } },
                motivos_movimiento: { select: { nombre: true } },
            },
    ...
    async update(empresaId: bigint, id: bigint, dto: UpdateMovimientoDto) {
        ...
        if (dto.lote_origen_id !== undefined) {
            data.lote_origen_id = dto.lote_origen_id ? parseBigInt(dto.lote_origen_id, 'lote_origen_id') : null;
        }
        if (dto.lote_destino_id !== undefined) {
            data.lote_destino_id = dto.lote_destino_id ? parseBigInt(dto.lote_destino_id, 'lote_destino_id') : null;
        }
        if (dto.potrero_origen_id !== undefined) {
            data.potrero_origen_id = dto.potrero_origen_id ? parseBigInt(dto.potrero_origen_id, 'potrero_origen_id') : null;
        }
        if (dto.potrero_destino_id !== undefined) {
            data.potrero_destino_id = dto.potrero_destino_id ? parseBigInt(dto.potrero_destino_id, 'potrero_destino_id') : null;
        }
        if (dto.id_motivo_movimiento !== undefined) {
            data.id_motivo_movimiento = dto.id_motivo_movimiento ? parseBigInt(dto.id_motivo_movimiento, 'id_motivo_movimiento') : null;
        }
    ...
    private mapMovimiento(m: Record<string, unknown>) {
        const mov = m as {
            ...
            lotes_movimientos_animales_lote_origen_id_empresa_idTolotes?: { nombre: string } | null;
            lotes_movimientos_animales_lote_destino_id_empresa_idTolotes?: { nombre: string } | null;
            potreros_movimientos_animales_potrero_origen_id_empresa_idTopotreros?: { nombre: string } | null;
            potreros_movimientos_animales_potrero_destino_id_empresa_idTopotreros?: { nombre: string } | null;
        };
        ...
        return {
            ...
            lote_origen_nombre: mov.lotes_movimientos_animales_lote_origen_id_empresa_idTolotes?.nombre ?? null,
            lote_destino_id: mov.lote_destino_id?.toString() ?? null,
            lote_destino_nombre: mov.lotes_movimientos_animales_lote_destino_id_empresa_idTolotes?.nombre ?? null,
            potrero_origen_id: mov.potrero_origen_id?.toString() ?? null,
            potrero_origen_nombre: mov.potreros_movimientos_animales_potrero_origen_id_empresa_idTopotreros?.nombre ?? null,
            potrero_destino_id: mov.potrero_destino_id?.toString() ?? null,
            potrero_destino_nombre: mov.potreros_movimientos_animales_potrero_destino_id_empresa_idTopotreros?.nombre ?? null,
