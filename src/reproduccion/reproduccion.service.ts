...
export class ReproduccionService {
  ...
  async findAll(empresaId: bigint, query: QueryEventoReproductivoDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.eventos_reproductivos.findMany({
        ...
        orderBy: { fecha: 'desc' },
        include: {
          animales_eventos_reproductivos_id_animal_empresa_idToanimales: {
            select: { nombre: true, id_animal: true },
          },
  ...
  async create(empresaId: bigint, dto: CreateEventoReproductivoDto) {
    ...
    const id_tipo = parseBigInt(
      dto.id_tipo_evento_reproductivo,
      'id_tipo_evento_reproductivo',
    );
    ...
    if (tipoEvento?.codigo === 'parto') {
      ...
      if (ultimoServicio && fecha <= ultimoServicio.fecha) {
        throw new BadRequestException(
          'La fecha de parto debe ser posterior a la fecha de servicio',
        );
      }
    }
    ...
    if (dto.id_resultado_palpacion) {
      data.id_resultado_palpacion = parseBigInt(
        dto.id_resultado_palpacion,
        'id_resultado_palpacion',
      );
    }
  ...
  async update(
    empresaId: bigint,
    id: bigint,
    dto: UpdateEventoReproductivoDto,
  ) {
    ...
    if (dto.reproductor_identificacion !== undefined)
      data.reproductor_identificacion = dto.reproductor_identificacion;
    if (dto.id_resultado_palpacion !== undefined) {
      data.id_resultado_palpacion = dto.id_resultado_palpacion
        ? parseBigInt(dto.id_resultado_palpacion, 'id_resultado_palpacion')
        : null;
    }
    if (dto.reproductor_id !== undefined) {
      data.reproductor_id = dto.reproductor_id
        ? parseBigInt(dto.reproductor_id, 'reproductor_id')
        : null;
    }
    if (dto.id_tipo_evento_reproductivo !== undefined) {
      data.id_tipo_evento_reproductivo = parseBigInt(
        dto.id_tipo_evento_reproductivo,
        'id_tipo_evento_reproductivo',
      );
    }
  ...
  async getSemaforo(empresaId: bigint) {
        ...
        eventos_reproductivos_eventos_reproductivos_id_animal_empresa_idToanimales:
          {
            ...
            take: 10,
            include: {
              tipos_evento_reproductivo: true,
              resultados_palpacion: true,
            },
          },
      },
    });
    ...
    return hembras.map((animal) => {
      const eventos =
        animal.eventos_reproductivos_eventos_reproductivos_id_animal_empresa_idToanimales;

      const ultimoServicio = eventos.find(
        (e) => e.tipos_evento_reproductivo.codigo === 'servicio',
      );
      const ultimaPalpacion = eventos.find(
        (e) => e.tipos_evento_reproductivo.codigo === 'palpacion',
      );
      const ultimoParto = eventos.find(
        (e) => e.tipos_evento_reproductivo.codigo === 'parto',
      );

      let diasAbiertos: number | null = null;
      if (ultimoParto) {
        diasAbiertos = Math.floor(
          (hoy.getTime() - ultimoParto.fecha.getTime()) / (1000 * 60 * 60 * 24),
        );
      }
      ...
      return {
        ...
        ultimo_servicio: ultimoServicio
          ? { fecha: ultimoServicio.fecha, detalles: ultimoServicio.detalles }
          : null,
        ultima_palpacion: ultimaPalpacion
          ? {
              fecha: ultimaPalpacion.fecha,
              resultado: ultimaPalpacion.resultados_palpacion?.nombre,
            }
          : null,
  ...
  private mapEvento(e: Record<string, unknown>) {
    ...
    return {
      ...
      animal_nombre:
        evento.animales_eventos_reproductivos_id_animal_empresa_idToanimales
          ?.nombre ?? null,
