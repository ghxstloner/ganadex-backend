...
CREATE TABLE `animales` (
    ...
    `id_color_pelaje` BIGINT UNSIGNED NULL,
    ...
    INDEX `idx_animales_color`(`id_color_pelaje`),
...
CREATE TABLE `eventos_reproductivos` (
    ...
    `proveedor_semen` VARCHAR(160) NULL,
    `codigo_pajuela` VARCHAR(120) NULL,
    `fecha_estimada_parto` DATE NULL,
    `control_21_dias` DATE NULL,
    `created_by` BIGINT UNSIGNED NULL,
    `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
...
-- CreateTable
CREATE TABLE `pesajes_leche_animales` (
    ...
    INDEX `idx_pla_animal_fecha`(`id_animal`, `empresa_id`, `fecha`),
    INDEX `idx_pla_created_by`(`created_by`),
    INDEX `idx_pla_turno`(`id_turno`),
    UNIQUE INDEX `uq_pla_animal_fecha_turno`(`empresa_id`, `id_animal`, `fecha`, `id_turno`),
...
-- CreateTable
CREATE TABLE `colores_pelaje` (
    `id_color` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `codigo` VARCHAR(40) NOT NULL,
    `nombre` VARCHAR(120) NOT NULL,

    UNIQUE INDEX `uq_colores_pelaje_codigo`(`codigo`),
...
CREATE TABLE `roles` (
    ...
    UNIQUE INDEX `uq_roles_empresa_codigo`(`empresa_id`, `codigo`),
    UNIQUE INDEX `uq_roles_empresa_nombre`(`empresa_id`, `nombre`),
    PRIMARY KEY (`id_rol`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
...
-- AddForeignKey
ALTER TABLE `animales` ADD CONSTRAINT `fk_animales_color` FOREIGN KEY (`id_color_pelaje`) REFERENCES `colores_pelaje`(`id_color`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `pesajes_leche_animales` ADD CONSTRAINT `fk_pla_animal` FOREIGN KEY (`id_animal`, `empresa_id`) REFERENCES `animales`(`id_animal`, `empresa_id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `pesajes_leche_animales` ADD CONSTRAINT `fk_pla_turno` FOREIGN KEY (`id_turno`) REFERENCES `turnos_ordenio`(`id_turno`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `pesajes_leche_animales` ADD CONSTRAINT `fk_pla_created_by` FOREIGN KEY (`created_by`) REFERENCES `usuarios`(`id_usuario`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `roles` ADD CONSTRAINT `fk_roles_empresa` FOREIGN KEY (`empresa_id`) REFERENCES `empresas`(`id_empresa`) ON DELETE CASCADE ON UPDATE CASCADE;
